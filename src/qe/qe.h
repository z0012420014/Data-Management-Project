        ...
        RC getNextTuple(void *data) {
        		while(input->getNextTuple(data) != QE_EOF){
        			...
        			if(condition.bRhsIsAttr){
        				...
        				/*
        				 * null attribute need to be recognize
        				 *
        				 * */
        				...
        				for(int j = 0; j < needAttrs.size(); j++){
        					...
        					if(allAttrs[i].name == needAttrs[j].name){
        						...
        						if(attr.type == TypeVarChar){
        							...
        							memcpy(((char*)data) + dataOffset, ((char*)recordData) + recordOffset - 4, length + sizeof(int));
        ...
        RC getNextTuple(void *data){
			...
			AfterFirstTimeLeft:
					...
					if(compare(attrType, condition.op)){
						...
						memset(data, 0, ceil((double) (leftAttrs.size() + rightAttrs.size()) / CHAR_BIT));
...
class Aggregate : public Iterator {
	...
	void calculate(void* value, void* data);
	bool isFind;
